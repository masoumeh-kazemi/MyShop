...
$(document).ready(function () {
    ...
    if (result) {
        ...
        if (result.Status === 200) {
            Success("", result.Message, result.IsReload);
        } else {
            ErrorAlert("", result.Message, result.IsReload);
        }
        deleteCookie("SystemAlert");
...
function OpenModal(url, name, title) {
    ...
    $.ajax({
        ...
        console.log(result);
        if (result.Status != 200) {
            ErrorAlert("مشکلی رخ داده است", result.Message);
            return;
        }
...
function CallBackHandler(result) {
    if (result.Status == 200) {
        Success(result.Title, result.Message, result.IsReload);
    } else {
        ErrorAlert(result.Title, result.Message, result.IsReload);
    }
